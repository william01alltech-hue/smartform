...
# 1. Add states
state_target = "const [cloudTemplates, setCloudTemplates] = useState<any[]>([]);"
state_replacement = """const [cloudTemplates, setCloudTemplates] = useState<any[]>([]);
  // Client Mode States
...
# 2. Add Client Mode Toggle to Header
header_target = """        <button onClick={() => setLanguage(language === 'zh' ? 'en' : 'zh')} style={{ padding: '6px 12px', borderRadius: '8px', background: 'rgba(255,255,255,0.1)', color: '#fff', border: '1px solid rgba(255,255,255,0.2)', cursor: 'pointer', fontSize: '14px' }}>
          {language === 'zh' ? 'EN' : '中文'}
        </button>"""
header_replacement = """        <button onClick={() => setIsClientMode(!isClientMode)} style={{ padding: '6px 12px', borderRadius: '8px', background: isClientMode ? '#10b981' : 'rgba(255,255,255,0.1)', color: '#fff', border: '1px solid rgba(255,255,255,0.2)', cursor: 'pointer', fontSize: '14px', fontWeight: 'bold' }}>
          {isClientMode ? '💻 切換至後台' : '📱 前台填寫模式'}
        </button>
        <button onClick={() => setLanguage(language === 'zh' ? 'en' : 'zh')} style={{ padding: '6px 12px', borderRadius: '8px', background: 'rgba(255,255,255,0.1)', color: '#fff', border: '1px solid rgba(255,255,255,0.2)', cursor: 'pointer', fontSize: '14px' }}>
          {language === 'zh' ? 'EN' : '中文'}
        </button>"""
content = content.replace(header_target, header_replacement)

# 3. Add Client Mode Render block right before <main className="main-layout">
main_target = "{/* Main dashboard content */}"
main_replacement = """
      {isClientMode ? (
        <div style={{ flex: 1, padding: '20px', maxWidth: '800px', margin: '0 auto', width: '100%', boxSizing: 'border-box', overflowY: 'auto' }}>
          {!clientFolder && !clientTemplate && (
            <div>
                  ...
                  <div key={idx} style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    ...
                    {field.type === 'text' && (
                      <input 
                        ...
                        placeholder="請輸入內容"
                        style={{ padding: '12px', borderRadius: '8px', background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', fontSize: '16px' }}
                      />
                    )}
                    ...
                    {field.type === 'dropdown' && (
                      ...
                      >
                        ...
                        {(field.dropdownOptions || '').split('\\n').filter(bool => bool).map((opt: string, i: number) => (
...
# 4. Wrap the end of main
end_main_target = "      </main>"
end_main_replacement = """      </main>
      )}"""
content = content.replace(end_main_target, end_main_replacement)
